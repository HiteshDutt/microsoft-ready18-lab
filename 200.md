# Hands on Lab - Deploy Java App using Jenkins for Web App for Containers 
...
### Generate SSH Key
    ...
    Create your public and private SSH keys on the command-line: 
    ...
    Make note: the location for the public key is at ~/.ssh/id_ras.pub. You will need the public key during Jenkins server installation. You can copy the key by cat ~/.ssh/id_ras.pub or vi to open it.

### Fork the Java app repo on github.com

    Fork the java app https://github.com/azure-devops/javawebappsample.
    In browser, go to the github repo page, sign in using your github.com account, then click "Fork" on repo page.

    Clone the app from your forked git repo to dev machine, on the command-line:
...
### Installl Jenkins Server
    ...
    The prcoess takes about 5-8 minutes depends on the size of VM you choose.

    Test if you can SSH to Jenikins server using the following command:
    
    ssh -L 127.0.0.1:8080:localhost:8080 [jenkins server link from portal]    
...
### Change the github repo for Jenkins Pipeline  
