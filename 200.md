...
### Generate SSH Key
    ...
    Make note: the location for the public key is at ~/.ssh/id_ras.pub. You will need the public key during Jenkins server installation. You can copy the key by cat ~/.ssh/id_ras.pub or use vi/vim to open it.
...
### Installl Jenkins Server
    ...
    ssh -L 127.0.0.1:8080:localhost:8080 username@serverdnsname.northcentralus.cloudapp.azure.com
    
    Note: the username is the user name you typed when created the Jenkins server, the serverdnsname is the Jenkins VM dns name (e.g. Domain name label), you can find it on the portal.    
